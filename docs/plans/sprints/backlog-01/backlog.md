...
and 32 pts of E10 operator-interaction work.**
...
## E10 — The operator decides, the conductor acts (32 pts)
...
| SK-144 | **`init --archive` cannot carry a backlog the operator never typed, so a bare `/supskill run <new-id>` cannot open a SCOPE sprint at all.** Step 3 tells the conductor to run `init <requested-id> --archive` with *"every `--entry`/`--backlog`/`--branch`/`--slug` flag from this invocation, verbatim"* (`SKILL.md:78`). An operator who typed `/supskill run s10` gave none; entry defaults to `SCOPE`; and `init_sprint` refuses SCOPE entry without `--backlog` (`commands.py:64`). The conductor had nothing to carry forward and stopped, correctly, with nothing mutated. The asymmetry is the defect: five lines below that refusal, `init` opens the state it is about to archive and inherits `continues` from it (`commands.py:91-97`, SK-116) — but never `backlog`, even though the archived state records one. And `backlog` is the single top-level field of `state.json` living *outside* the `sprint` object — project-scoped by its own placement — yet the only one reset to `null` at every init. This is the uncovered half of SK-114: that row fixed *the template drops the operator's `--backlog`*; nobody covered *the operator typed no `--backlog`, because they never needed to*. SK-114 predicted this failing the other direction — an init that loses the flag starts with `backlog: null` and `replan-guard` refuses at Gate 3 — and the up-front refusal is the better failure, but both grow from the same root SK-114 named: the target is deduced from the invocation, never read from the trail. Fix: when `--archive` archives a state that records a backlog and no `--backlog` is given, inherit it, mirroring SK-116's reasoning verbatim — don't make the conductor re-supply a fact the trail already holds. An explicit `--backlog` still wins, and *"no verb can set it after init"* is untouched, since this resolves **at** init, from disk. Then rewrite step 3's template so it stops implying the invocation is the only source. Note the operator-facing cost: a resume that invariant 5 promises is `/clear` + re-invoke now requires the operator to know a path that is already on disk. (ledgerus s9 — REVIEW, `G3 replan`, 14/14 DONE, the exact `init.archive.decided` shape E10 made remediable; 2026-08-04, immediately after the 0.7.0 install; reproduced offline.) | 2 | M | ☐ |
